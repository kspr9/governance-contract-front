    ...
    async function loadContractTzkt() {
        try {
            ...
            */
            ...
            console.log('TzKT Storage Data:', $state.snapshot(tzktStorageData));
            console.log('Processed Entries:', {
                owners: $state.snapshot(tzktOwnersMapEntries),
                unclaimed: $state.snapshot(tzktUnclaimedSharesEntries),
                balances: $state.snapshot(tzktShareBalancesEntries)
            });
        } catch (error) {
